...
class LoL_char {
  ...
  String[] user_create() {
    //TODO:
    String[] charArr = new String[4];

    System.out.print("[시스템] 유닛 [이름] 을 입력해 주세요 :");
    charArr[0] = scanner.nextLine();
    System.out.print("[시스템] 유닛 [공격력] 을 입력해 주세요 : (Ex 50)");
    charArr[1] = scanner.nextLine();
    System.out.print("[시스템] 유닛 [방어력] 을 입력해 주세요 : (Ex 1)");
    charArr[2] = scanner.nextLine();
    System.out.print("[시스템] 유닛 [체력] 을 입력해 주세요 : (Ex 100)");
    charArr[3] = scanner.nextLine();

    user_info(charArr[0], charArr[1], charArr[2], charArr[3]);
  ...
  int[] user_info_int(String[] info) {
    ...
    for (int i = 0; i < info.length - 1; i++) {
      intArr[i] = Integer.parseInt(info[i + 1]);
    }
    return intArr;
  }
